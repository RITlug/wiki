...
This is the complete instruction list for configuring `iwd`. These instructions use the `PEM` certificate. 
...
6. Choose a format to download your certificate in. Select PEM format and save the file somewhere you can get to it easily.
...
8. Run the following command to determine what network interfaces you have available. This tutorial assumes a network interface of `wlan0`.
```
# ip link
```
...
9. Move both the RIT CA Cert and the encrypted `.pem` file into the following directory: `/var/lib/iwd`. This can be done by running the following command:
```
...
10. In the same directory, create a configuration file named `eduroam.8021x`. In it, input the following information:
```
[Security]
EAP-Method=TLS
...
11. Run the following command as root: 
```
...
12. (Optional) Run the following to ensure that your connection is working: 
```
ping 9.9.9.9
```
